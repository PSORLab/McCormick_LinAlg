...
#::SVector{n, MC{N}}
function XNRM2(X::Vector) #x,y E(Vector(MC{N})) where N <: Integer
    n::Int = length(X)
    ...
    if m != 0
        ...
        #= #If multtemp returns list of values
           temp::SVector{6,Any} = multtemp(X[i], Y[i])
            ...
            temp1 = X[i]^2 #Mult function needs more integration here
        ...
        #= #If multtemp returns list of values
           ...
           cum_cnst = (cum_cnst && temp1.cnst && temp2.cnst && temp3.cnst && temp4.cnst && temp5.cnst) #pointer?
       end
    ...
    return sqrt(result) #best way to sqrt this?
end
